...
export class UsersService {
    ...
    async loginUser(email: string, password: string, fingerprint?: string | null): Promise<{ token: string }> {
        try {
            ...
            if (fingerprint) {
                const device = this.deviceService.findByFingerprint(fingerprint)
                if (!device) {
            ...
            const accessToken = jwt.sign(payload, secretKey, { expiresIn: appconfig.auth.jwtExpiration, algorithm: appconfig.auth.jwtAlgorithm, issuer: appconfig.auth.jwtIssuer } as SignOptions); //TODO: use appconfig for the token options and make sure to set appropriate expiration time, algorithm, and issuer for the tokens to improve security
            return {
                token: accessToken
            }
